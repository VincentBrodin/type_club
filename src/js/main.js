...
function OnInput() {
    ...
    if (inputLen === targetLen) {
        ...
        const mins = secs / 60;
        let words = target.split(" ").length;
        //The avrage word length is 4.7 so this is more avrage
        let avrageWords = target.length / 4.7;
        let wpm = words / mins;
        let awpm = avrageWords / mins;
...
function SetOutput(text, target) {
    ...
    let inputWords = input.value.split(" ").length - 1;
    let targetWords = target.split(" ").length;
